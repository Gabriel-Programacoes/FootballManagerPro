                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            <DialogTitle className="text-2xl flex items-center gap-4">
                                <span>{player.attributes.profile.nation} {player.name}</span>
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem><ArrowRightLeft className="mr-2 h-4 w-4" /><span>Listar para Transferência</span></DropdownMenuItem>
                                <DropdownMenuSub>
                                    <DropdownMenuSubTrigger><Send className="mr-2 h-4 w-4" /><span>Listar para Empréstimo</span></DropdownMenuSubTrigger>
                                    <DropdownMenuSubContent>
                                        <DropdownMenuItem>1 Ano</DropdownMenuItem>
                                        <DropdownMenuItem>2 Anos</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem>+ Opção de Compra</DropdownMenuItem>
                                        <DropdownMenuItem>+ Obrigação de Compra</DropdownMenuItem>
                                    </DropdownMenuSubContent>
                                </DropdownMenuSub>
                                <DropdownMenuSeparator />
                                <DropdownMenuLabel>Contrato</DropdownMenuLabel>
                                <DropdownMenuItem><FileSignature className="mr-2 h-4 w-4" /><span>Renovar Contrato</span></DropdownMenuItem>
                                <AlertDialog>
                                    <AlertDialogTrigger asChild><DropdownMenuItem onSelect={(e) => e.preventDefault()} className="text-red-500 focus:bg-red-500/10 focus:text-red-600"><X className="mr-2 h-4 w-4" /><span>Rescindir Contrato</span></DropdownMenuItem></AlertDialogTrigger>
                                    <AlertDialogContent>
                                        <AlertDialogHeader><AlertDialogTitle>Tem a certeza?</AlertDialogTitle><AlertDialogDescription>Rescindir o contrato de {player.name} terá um custo financeiro significativo e o jogador deixará o clube imediatamente. Esta ação não pode ser desfeita.</AlertDialogDescription></AlertDialogHeader>
                                        <AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction className="bg-destructive hover:bg-destructive/90">Confirmar Rescisão</AlertDialogAction></AlertDialogFooter>
                                    </AlertDialogContent>
                                </AlertDialog>
                            </DropdownMenuContent>
                        </DropdownMenu>
                            ...
                            <div className="flex justify-between items-center text-sm">
                                ...
                                {!isEditingNumber ? (
                                    ...
                                    <div className="flex items-center gap-1">
                                        <Input value={jerseyNumber} onChange={(e) => setJerseyNumber(e.target.value)} className="h-8 w-16 text-center" maxLength={2} />
                                        <Button variant="ghost" size="icon" className="h-7 w-7 text-green-500" onClick={() => setIsEditingNumber(false)}><Check className="h-4 w-4" /></Button>
                                        <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500" onClick={() => setIsEditingNumber(false)}><Ban className="h-4 w-4" /></Button>
                                    </div>
                                )}
                            </div>
                        </div>
                        <div className="space-y-2 text-sm border-t pt-2">
                            <div className="flex justify-between"><span>Idade:</span> <span className="font-semibold">{player.age}</span></div>
                            <div className="flex justify-between"><span>Pé Preferido:</span> <span className="font-semibold">{player.attributes.mentality.preferredFoot}</span></div>
                        </div>
                        <div className="space-y-2 text-sm border-t pt-2">
                            <div className="flex justify-between"><span>Valor:</span> <span className="font-semibold font-mono">€ {formatCompactNumber(player.contract.value)}</span></div>
                            <div className="flex justify-between"><span>Salário:</span> <span className="font-semibold font-mono">€ {formatCompactNumber(player.contract.wage)}/sem</span></div>
                        </div>
                        <div className="space-y-3 border-t pt-3">
                            <div className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                                <label htmlFor="block-proposals" className="text-sm font-medium flex items-center gap-2"><ShieldOff className="h-4 w-4" /> Bloquear Propostas</label>
                                <Switch id="block-proposals" checked={blockProposals} onCheckedChange={setBlockProposals} />
                            </div>
                        </div>
                    </div>
